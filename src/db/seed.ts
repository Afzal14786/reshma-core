...
const DUMMY_IMAGE = 'https://res.cloudinary.com/demo/image/upload/v1312461204/sample.jpg';
...
const generateProducts = (template: any, count: number, prefix: string) => {
    ...
    for (let i = 1; i <= count; i++) {
        ...
        products.push({
            ...
            tags: [template.mainCategory.toLowerCase(), template.material.toLowerCase(), `variant${i}`],
        });
    }
    return products;
};
...
const seedDatabase = async () => {
    try {
        ...
        console.log('💾 Inserting into Database (Mongoose Discriminators Active)...');
