...
import { AddItemToCartInput, UpdateCartItemInput, MergeCartInput } from './dtos/cart.dto';
...
export class CartService {
     ...
     */
    private static generateItemSignature(productId: string, attributes?: Record<string, AttributeValue>): string {
    ...
    public static async getCart(userId: string) {
        ...
        let cart = await Cart.findOne({ user: userId }).populate({
            path: 'items.product',
            select: 'name sku basePrice discount currentStock weightGrams isFragile isActive images itemType'
        });

        if (!cart) {
            cart = await Cart.create({ user: userId, items: [] });
            return { cart, totals: { subTotal: 0, totalWeightGrams: 0, hasFragileItems: false, totalItems: 0 } };
        }

        let needsSave = false;
        const validItems = [];
        const totals: ICartTotals = { subTotal: 0, totalWeightGrams: 0, hasFragileItems: false, totalItems: 0 };
        ...
        for (const item of cart.items) {
            ...
            const activePrice = product.basePrice - (product.basePrice * (product.discount / 100));
    ...
    public static async addItem(userId: string, payload: AddItemToCartInput) {
        ...
        const product = await Product.findById(productId).select('currentStock isActive').lean();
        
        if (!product || !product.isActive) {
            throw new AppError(HTTP_STATUS.NOT_FOUND, 'This product is no longer available.');
        }

        if (product.currentStock < quantity) {
            throw new AppError(HTTP_STATUS.CONFLICT, `Only ${product.currentStock} items left in stock.`);
        }
        ...
        const incomingSignature = this.generateItemSignature(productId, selectedAttributes as Record<string, AttributeValue>);
        
        const existingItemIndex = cart.items.findIndex((item) => {
            const itemSignature = this.generateItemSignature(item.product.toString(), item.selectedAttributes as Record<string, AttributeValue>);
        ...
        if (existingItemIndex > -1) {
            ...
            if (existingItem) {
                ...
                if (newQuantity > product.currentStock) {
                    throw new AppError(HTTP_STATUS.CONFLICT, `Cannot add more. Stock limit reached (${product.currentStock}).`);
                }
            ...
            const newItemObject: Partial<ICartItem> = {
                ...
                quantity,
                ...(selectedAttributes ? { selectedAttributes: selectedAttributes as Record<string, AttributeValue> } : {})
            };
     ...
     */
    public static async updateItemQuantity(userId: string, payload: UpdateCartItemInput) {
        const { productId, quantity, selectedAttributes } = payload;

        if (quantity === undefined) {
            throw new AppError(HTTP_STATUS.BAD_REQUEST, 'Quantity is required for this operation.');
        }

        const cart = await Cart.findOne({ user: userId });
        if (!cart) throw new AppError(HTTP_STATUS.NOT_FOUND, 'Cart not found.');

        const targetSignature = this.generateItemSignature(productId, selectedAttributes as Record<string, AttributeValue>);
        
        const itemIndex = cart.items.findIndex(item => 
            this.generateItemSignature(item.product.toString(), item.selectedAttributes as Record<string, AttributeValue>) === targetSignature
        );

        if (itemIndex === -1) {
            throw new AppError(HTTP_STATUS.NOT_FOUND, 'Item variant not found in cart.');
        }

        // Real-time stock verification
        const product = await Product.findById(productId).select('currentStock isActive').lean();
        if (!product || !product.isActive || product.currentStock < quantity) {
            throw new AppError(HTTP_STATUS.CONFLICT, `Requested quantity exceeds available stock (${product?.currentStock || 0}).`);
        }
     ...
     */
    public static async mergeGuestCart(userId: string, guestItems: MergeCartInput['items']) {
        ...
        for (const guestItem of guestItems) {
            ...
            const product = await Product.findById(guestItem.productId).select('currentStock isActive').lean();
            ...
            const existingItemIndex = cart.items.findIndex(item => 
                this.generateItemSignature(item.product.toString(), item.selectedAttributes as Record<string, AttributeValue>) === incomingSignature
            );
            ...
            if (existingItemIndex > -1) {
                ...
                if (existingItem) {
                    ...
                    existingItem.quantity = Math.min(combinedQuantity, product.currentStock);
                }
            } else {
                ...
                const newItemObject: Partial<ICartItem> = {
                    ...
                    ...(guestItem.selectedAttributes ? { selectedAttributes: guestItem.selectedAttributes as Record<string, AttributeValue> } : {})
                };
                cart.items.push(newItemObject as any);
            }
        }
