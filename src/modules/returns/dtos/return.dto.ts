...
export const ArbitrateReturnSchema = z.object({
  ...
  .refine(
    (data) => {
      ...
      if (data.status === ReturnStatus.REJECTED && !data.adminRejectionReason) {
        return false;
      }
      return true;
    },
    {
      message: "adminRejectionReason is legally required when rejecting a return",
...
export type ArbitrateReturnInput = z.infer<typeof ArbitrateReturnSchema>["body"];