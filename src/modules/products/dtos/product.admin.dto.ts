...
const BaseProductRules = {
    sku: z.string().min(3, "SKU must be at least 3 characters").trim().toUpperCase(),
    ...
    weightGrams: z.number().min(0, "Weight is required for shipping calculations"),
    isFragile: z.boolean(),
...
const BangleSchema = BaseProductSchema.extend({
    ...
    bangleSizes: z.array(z.enum(['2.2', '2.4', '2.6', '2.8'])).min(1, "At least one size is required"),
    packSize: z.number().int().min(1).default(12),
});

const ApparelSchema = BaseProductSchema.extend({
    itemType: z.literal('APPAREL'),
    sizes: z.array(z.enum(['XS', 'S', 'M', 'L', 'XL', 'XXL', 'Free Size', '34', '36', '38', '40'])).min(1),
...
export const UpdateProductSchema = z.object({
    ...
    body: BaseProductSchema.extend({
        ...
        sizes: z.array(z.enum(['XS', 'S', 'M', 'L', 'XL', 'XXL', 'Free Size', '34', '36', '38', '40'])),
