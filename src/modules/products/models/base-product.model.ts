...
const BaseProductSchema = new Schema<IBaseProduct>({
    itemType: { type: String, required: true, enum: ['BANGLE', 'APPAREL', 'FABRIC', 'INNERWEAR', 'ACCESSORY'] },
    sku: { type: String, required: [true, 'SKU is required'], unique: true, trim: true, uppercase: true },
    name: { type: String, required: [true, 'Product name is required'], trim: true },
    mainCategory: { type: String, required: true, enum: ['Sarees', 'Apparel', 'Accessories', 'Innerwear', 'Bangles'] },
    subCategory: { type: String, required: true, trim: true },
    material: { type: String, required: true, trim: true },
    sellingUnit: { type: String, required: true, enum: ['Single Piece', 'Meter', 'Set', 'Pair', 'Dozen', 'Pack'] },
    ...
    images: { 
        type: [String], 
        required: true,
        validate: { validator: (arr: string[]) => arr.length > 0, message: 'Minimum one image required' }
    },
...
BaseProductSchema.index({ isActive: 1, itemType: 1, mainCategory: 1, createdAt: -1 });

export const Product = mongoose.model<IBaseProduct>('Product', BaseProductSchema);