...
process.on("uncaughtException", (err: Error) => {
    logger.error("UNCAUGHT EXCEPTION! Shutting down forcefully...", { error: err.message, stack: err.stack });
    process.exit(1);
});
...
const bootstrap = async () => {
    try {
        ...
        server = app.listen(env.PORT, () => {
            logger.info(`Reshma-Core API is running on port ${env.PORT} in ${env.NODE_ENV} mode.`);
        });
...
process.on("unhandledRejection", (err: Error) => {
    logger.error("UNHANDLED REJECTION! Initiating graceful shutdown...", { error: err.message, stack: err.stack });
    gracefulShutdown();
});
